...
## 2026-03-21 - [Keyboard Shortcuts Help UX/Accessibility Enhancements]
**Learning:** Interactive elements within utility modals (like the Command Palette) require high-clarity affordances. Icon-only buttons (e.g., "Close") must be wrapped in `Tooltip` for context. Searchable lists need a dedicated "No results" state to prevent "blank screen" confusion. Tactical feedback (haptics) on toggles (like Vim mode) provides delightful physical confirmation on mobile. Finally, `backdrop-blur-sm` on modal overlays significantly improves focus by visually receding background content.
**Action:** Implement "No results" states for all searchable UI. Wrap icon-only modal actions in `Tooltip`. Apply `backdrop-blur-sm` to overlays for enhanced focus. Use haptics for significant UI state toggles.
