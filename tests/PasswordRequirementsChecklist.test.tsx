...
describe('PasswordRequirementsChecklist', () => {
  ...
  it('renders correctly when password is empty and showWhenEmpty is true', () => {
    ...
    expect(group).toHaveAttribute(
      'aria-label',
      'Password requirements: 0 of 5 met'
    );
    ...
    const uppercaseItem = screen.getByLabelText(
      /Contains uppercase letter: not met/i
    );
    expect(uppercaseItem).toBeInTheDocument();

    const lowercaseItem = screen.getByLabelText(
      /Contains lowercase letter: not met/i
    );
    ...
    const specialItem = screen.getByLabelText(
      /Contains special character \(!@#\$%\^&\*\): not met/i
    );
  ...
  it('correctly updates met status when partial password is provided', () => {
    ...
    render(
      <PasswordRequirementsChecklist password="Ab" showWhenEmpty={true} />
    );

    const group = screen.getByRole('group');
    expect(group).toHaveAttribute(
      'aria-label',
      'Password requirements: 2 of 5 met'
    );
    ...
    expect(
      screen.getByLabelText(/Contains uppercase letter: met/i)
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Contains lowercase letter: met/i)
    ).toBeInTheDocument();

    // Unmet requirements: length, number, special
    expect(
      screen.getByLabelText(/At least 8 characters: not met/i)
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Contains a number: not met/i)
    ).toBeInTheDocument();
    expect(
  ...
  it('shows success and celebration when all requirements are met', () => {
    ...
    render(
      <PasswordRequirementsChecklist password="Ab1!cdef" showWhenEmpty={true} />
    );

    const group = screen.getByRole('group');
    expect(group).toHaveAttribute(
      'aria-label',
      'Password requirements: 5 of 5 met'
    );
    ...
    expect(
      screen.getByLabelText(/Contains a number: met/i)
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText(/Contains special character \(!@#\$%\^&\*\): met/i)
    ).toBeInTheDocument();
