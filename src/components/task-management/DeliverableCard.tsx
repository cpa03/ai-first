...
interface DeliverableCardProps {
  ...
  onToggleExpand: (_deliverableId: string) => void;
  onToggleTask: (_taskId: string, _currentStatus: TaskStatus) => Promise<void>;
}
...
function DeliverableCardComponent({
  ...
  const inlineProgressFillClasses = useMemo(() => {
    ...
    if (
      deliverable.progress >=
      COMPONENT_CONFIG.DELIVERABLE_CARD.PROGRESS_THRESHOLDS.HIGH
    )
      return PROGRESS_BAR_COLORS.HIGH;
    if (
      deliverable.progress >=
      COMPONENT_CONFIG.DELIVERABLE_CARD.PROGRESS_THRESHOLDS.MEDIUM
    )
      return PROGRESS_BAR_COLORS.MEDIUM;
