...
function ReferralLinkComponent({
  ...
  return (
    <div
      className={`${GRADIENT_CONFIG.REFERRAL_BACKGROUND} rounded-lg p-4 border ${PRIMARY_BORDER._100} ${className}`}
      role="region"
    ...
    >
      <div className={RESPONSIVE_FLEX_BETWEEN_GAP}>
        <div className={FLEX_GROW_PATTERNS.GROW}>
          <h3
            className={`${REFERRAL_LINK_STYLES.TITLE} ${COMPONENT_PRIMARY_PATTERNS.REFERRAL_TITLE}`}
          >
          ...
          <div className={COMMON_SPACING_PATTERNS.FLEX_CENTER_SM}>
            ...
            >
              <code
                ...
                className={`w-full min-w-0 px-3 py-2 ${WHITE_BG_PATTERNS.DEFAULT} border ${PRIMARY_BORDER._200} rounded-md text-sm ${PRIMARY_TEXT._800} truncate font-mono cursor-pointer ${GRAY_CLASSES.HOVER_BG_50} active:scale-[0.98] transition-all ${DURATION_TAILWIND[200]} outline-none ${FOCUS_RING_PATTERNS.DEFAULT}`}
              >
                {referralUrl}
              </code>
            </Tooltip>
