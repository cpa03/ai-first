...
/**
 * Lazily retrieves the admin API key from environment variables.
 * PERFORMANCE/SECURITY: This avoids accessing process.env and logging warnings
 * during module load time, which is required for stable "Workers Builds"
 * and Edge runtime compatibility.
 */
let _cachedAdminKey: string | undefined = undefined;
let _adminKeyInitialized = false;

function getAdminApiKey(): string | undefined {
  if (!_adminKeyInitialized) {
    _cachedAdminKey = process.env[SECURITY_ENV_KEYS.ADMIN_API_KEY];
    _adminKeyInitialized = true;
