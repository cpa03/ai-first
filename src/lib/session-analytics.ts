...
function getSessionId(): string {
  ...
  try {
    ...
    if (!sessionId) {
      // SECURITY: Use generateSecureId() for cryptographically secure session IDs
...
function flushEvents(): void {
  ...
  // SECURITY: Use logger.debug instead of console.log to prevent potential sensitive data leakage
  // and comply with production logging standards.
  if (process.env.NODE_ENV !== 'production') {
    logger.debug(
