...
import { simpleHash } from './security/id-generator';
...
function extractUserIdFromRequest(request: Request): string | null {
  ...
  if (authHeader && authHeader.toLowerCase().startsWith('bearer ')) {
    ...
    if (token && token.length > 0) {
      ...
      // SECURITY: Avoid using substrings of the token which could leak secret data
...
function generateRequestFingerprint(request: Request): string {
  ...
  // Use centralized DJB2 hash algorithm for stable fingerprints
