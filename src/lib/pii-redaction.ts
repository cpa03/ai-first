...
function _redactPII(text: string): string {
  ...
  const minLens = PII_REDACTION_CONFIG.MIN_LENGTHS;
  ...
  if (originalLength >= minLens.JWT && redacted.includes('eyJ')) {
  ...
  if (/\d/.test(redacted)) {
    ...
    if (
      originalLength >= minLens.IP_ADDRESS &&
  ...
  if (
    originalLength >= minLens.API_KEY &&
    (/[:=]/.test(redacted) ||
...
export function redactPIIInObject(
  ...
  if (Array.isArray(obj)) {
    ...
    for (let i = 0; i < obj.length; i++) {
      const item = obj[i];
      // PERFORMANCE: Inline fast-path for non-string primitives
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    try {
      // SECURITY: Check action BEFORE accessing value to avoid triggering
      // potentially sensitive or expensive getters.
  ...
  // PERFORMANCE: Guard symbol processing with length check
