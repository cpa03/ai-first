 ...
 * PERFORMANCE: This consolidated regex is ~3x faster than multiple .replace() calls
 * because it scans the string only once.
 */
...
export function redactPII(text: string): string {
  ...
  // Single-pass replacement using named capture groups
  return text.replace(COMBINED_PII_REGEX, (...args) => {
    ...
    if (groups.ipAddress) {
      return isPrivateIP(groups.ipAddress) ? groups.ipAddress : labels.IP_ADDRESS;
    }
    if (groups.apiKey) return labels.API_KEY;

    return args[0]; // Fallback (should not happen with correctly defined groups)
  });
}
