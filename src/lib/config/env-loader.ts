/**
 * EnvLoader — type-safe environment variable loader (leaf module).
 *
 * Lives in its own file so domain config modules can import it WITHOUT
 * creating import cycles through `environment.ts` (which aggregates the
 * canonical TIMEOUT / RATE_LIMIT / RETRY / CACHE configs for ENV_CONFIG).
 *
 * `environment.ts` re-exports this class, so existing
 * `import { EnvLoader } from './env-loader'` paths keep working — but new
 * config modules MUST import from './env-loader' directly.
 */
