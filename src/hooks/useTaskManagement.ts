...
/**
 * PERFORMANCE: Pure helper function moved outside the hook to ensure it has a static identity.
 * This prevents the recreation of callbacks that depend on it and simplifies the hook logic.
 */
...
export function useTaskManagement(ideaId: string): UseTaskManagementReturn {
  ...
  // PERFORMANCE: Stabilize callbacks by updating ref in useEffect rather than render phase.
  // This avoids the react-hooks/refs lint error and ensures consistent state lookups.
  useEffect(() => {
