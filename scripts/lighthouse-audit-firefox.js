#!/usr/bin/env node
/**
 * BroCula Lighthouse Auditor (Using Playwright Firefox)
 * Runs Lighthouse audits on all pages using Firefox
 */
...
async function runLighthouse(url) {
  // Try to find any available Chrome/Chromium
  const possiblePaths = [
    CHROME_PATH,
  ...
  try {
    if (chromePath) {
      ...
      console.log('  No Chrome found, trying auto-detection...');
      chrome = await chromeLauncher.launch({
        chromeFlags: [...CHROME_FLAGS.COMMON, ...CHROME_FLAGS.LIGHTHOUSE],
      });
    }
